...
const TravelOverlay = () => {
    ...
    const progressPercent = useMemo(() => Math.min(100, (elapsed / jumpDuration) * 100), [elapsed, jumpDuration]);
    const distance = useMemo(() => {
        const dx = jumpToCoord.x - jumpFromCoord.x;
        const dy = jumpToCoord.y - jumpFromCoord.y;
        const dz = jumpToCoord.z - jumpFromCoord.z;
        return Math.round(Math.sqrt(dx * dx + dy * dy + dz * dz));
    }, [jumpFromCoord, jumpToCoord]);
    // star map scaling
    const coordsList = galaxiesData.galaxies.map(g => g.coordinates);
    const xs = coordsList.map(c => c.x), ys = coordsList.map(c => c.y);
    ...
    return (
        <div className="travel-overlay" style={{ 
            position: 'fixed', 
            top: '50%', 
            left: '50%', 
            ...
            {showDeparture ? (
                ...
                <div style={{ width: '80%', background: '#444', borderRadius: 4, overflow: 'hidden', marginBottom: 20 }}>
                    <div style={{ width: `${progressPercent}%`, height: 10, background: '#0f0' }} />
                </div>
            )}
            <div style={{ fontSize: 14, marginBottom: 10 }}>Distance: {distance}</div>
            <svg width={width} height={height} style={{ background: '#111', border: '1px solid #555' }}>
                {galaxiesData.galaxies.map(g => {
                    const x = Math.max(25, Math.min(width - 25, (g.coordinates.x - minX + 25) * scaleX));
                    const y = (g.coordinates.y - minY) * scaleY;
                    const isFrom = g.coordinates.x === jumpFromCoord.x && g.coordinates.y === jumpFromCoord.y;
                    const isTo = g.coordinates.x === jumpToCoord.x && g.coordinates.y === jumpToCoord.y;
                    return (
                        <circle key={`galaxy-${g.galaxyId}`}
                            cx={x} cy={y} r={isFrom || isTo ? 5 : 2}
