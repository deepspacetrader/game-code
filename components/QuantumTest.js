  ...
  const runAllTests = async () => {
    ...
    for (const test of availableTests) {
      ...
      try {
        ...
        results[test.id] = [{ name: 'Test suite failed to load', passed: false, error: error.message }];
      }
    }
    ...
    <div className="quantum-test-container">
      ...
      <div className="test-suites">
        ...
        <ul>
          {getAvailableTests().map((test) => (
            <li key={test.id} className={`test-suite ${test.canRun ? 'available' : 'unavailable'}`}>
      ...
      <div className="test-log">
        ...
        <div className="log-entries">
          ...
          {testLog.length === 0 && (
            <div className="log-entry">No test output yet. Click 'Run All Tests' to start.</div>
          )}
        </div>
      </div>
    </div>
