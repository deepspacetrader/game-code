...
const QuantumSetup = ({ setQuantumSlotsUsed, setStatusEffects }) => {
    const { inventory, subtractQuantumProcessor } = useMarketplace();
    ...
    const [slots, setSlots] = useState(() => {
        const initialSlots = Array(slotsCount).fill(null).map((_, index) => {
            if (index === slotsCount - 1) {
                return { active: false, ability: 'QuantumMarket' };
            }
            
            // Randomly assign a scan direction
            const scanTypes = ['QuantumScanLR', 'QuantumScanTB', 'QuantumScanRL', 'QuantumScanBT'];
            ...
            .map((_, i) => (
                <FontAwesomeIcon
                    ...
                    style={{
                        color:
                            i < power
                                ? QUANTUM_ABILITIES[slots[i]]?.color || '#fff'
                                : '#555',
                    }}
                />
            ));
    ...
    const toggleSlotActivation = useCallback(async (index) => {
    ...
    }, [quantumCount, quantumProcessor, subtractQuantumProcessor, setStatusEffects, setQuantumSlotsUsed, slots]);
