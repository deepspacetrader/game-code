        ...
        <div className="trade-history">
            {improvedUILevel < 50 && (
                <ul className="history-list">
                    {historyArr.map((t, i) => (
                        <li key={i}>
                            {t.type === 'buy' ? 'Bought' : 'Sold'} {t.quantity}×{t.name} for {t.quantity * t.price}{' '}
                            <span className={`trade-profit ${t.profit >= 0 ? 'positive' : 'negative'}`}>
                                ({t.profit >= 0 ? '+' : ''}{t.profit})
                            </span>
                        </li>
                    ))}
                    ...
                    <div className="history-detail" ref={scrollRef}>
                        {historyArr.map((t, i) => (
                            <div key={i} className="history-item-detailed">
                                {new Date(t.time).toLocaleTimeString()} {t.type === 'buy' ? 'Bought' : 'Sold'} {t.quantity}×{t.name} for {t.quantity * t.price}{' '}
                                <span className={`trade-profit ${t.profit >= 0 ? 'positive' : 'negative'}`}>
                                    ({t.profit >= 0 ? '+' : ''}{t.profit})
                                </span>
                            </div>
