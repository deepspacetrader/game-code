...
const useEventSystem = (triggerEvent) => {
    ...
    const processEventEffects = useCallback((event) => {
        ...
        const priceMultiplier = randomFloat(priceMultiplierRange[0], priceMultiplierRange[1]);
        const stockMultiplier = randomFloat(stockMultiplierRange[0], stockMultiplierRange[1]);
    ...
    const checkForEvent = useCallback(() => {
        ...
        const eligibleEvents = breakingNews.events.filter(
            (e) => !activeEvents.some((ae) => ae.randomEventId === e.randomEventId)
        );
    ...
    const cleanupEvents = useCallback(() => {
        const now = Date.now();
        setActiveEvents((prev) => prev.filter((event) => event.endTime > now));
    }, []);
